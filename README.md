...
| 区块 | 内容 |
|------|------|
| Jev API Key | 凭据名 + 配置状态（已配置/未配置 + 来源）+ 密码输入框 + 保存/清除 |
...
1. **配 Key**：设置 → 插件 → Jev 路由 → 「Jev API Key」粘贴 TypeSafe API Key → 保存。保存后**下一次对话即生效**，不需要再重启。
2. **确认生效**：发 `/jev status` 或 `/jev why`，看判定的**来源**——`jev(jev-1.13.0)` 表示走真 Jev；`heuristic` 表示没配 Key 或调用失败（超时/限流）。
