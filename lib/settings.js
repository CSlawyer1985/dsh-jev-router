...
function buildConfig() {
  ...
  return z.object({
    ...
    downgradeStreak: tunable(
      ...
      '降档需要连续判定为低档的轮数。只作用于「高错误代价」的降档；低代价降档立即生效。',
    ),
    offFloorChars: tunable(
      z.number().step(1).min(0).default(DEFAULTS.offFloorChars),
      'off 地板的字符阈值：消息不短于此长度时不允许降到 off（0 = 关闭）。off 的语义是"琐碎到不需要思考"，需要"短消息"这个正面证据；而 Jev 对中文的准确率官方说明较低。',
    ),
    syncSessionEffort: tunable(
      z.boolean().default(DEFAULTS.syncSessionEffort),
      '把生效档位同步进会话配置，让 DSH 原生的「模型后面的思考程度」跟随变化（否则它永远停在会话设置上，你看不到插件在工作）。代价：会覆盖你在原生选择器里的手动选择，且每次变化往会话日志追加一条事件。',
    ),
    riskCeiling: tunable(
      z.number().min(0).max(3).default(DEFAULTS.riskCeiling),
      '「低错误代价」上界（Jev 的 risk 分 0-3）。risk 不超过此值的降档立即生效，不必等连续确认——这样每条消息都能立刻看到效果，同时高代价降档仍受保护。',
    ),
    timeoutMs: tunable(
    ...
    contextSafetyMargin: tunable(
      z.number().min(0).max(0.5).default(DEFAULTS.contextSafetyMargin),
      '切换模型前的上下文窗口安全余量（比例）。比较「当前会话已用 token」与「目标模型窗口×(1-余量)」，装不下就拒绝切换——因为那会让请求必然失败（实测：556K 会话切到 272K 窗口的模型直接被拒）。0 = 用满窗口。',
    ),
