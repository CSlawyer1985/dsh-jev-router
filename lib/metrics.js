...
export function createMetrics({ windowSize = 50 } = {}) {
  return {
    ...
    auxiliaryCalls: 0,
    /**
     * provider 是否**上报过** reasoningTokens。
     *
     * 实测：当前 DSH 版本里这个字段「定义了但没有任何适配器填充」——
     * DeepSeek / pi-ai / Anthropic / OpenAI 都不填。因此不能把「0」当成
     * 「没有思考」，否则界面会长期显示一个假的 0%。
     */
...
export function recordUsage(metrics, usage, meta = {}) {
  ...
  // 只要有一次 usage 真的带了这个字段，就算「该 provider 会上报」。
  if (typeof usage.reasoningTokens === 'number') metrics.reasoningReported = true;
...
export function snapshot(metrics, price = null) {
  ...
  // 未上报时给 null（界面显示「未上报」），而不是伪造一个 0%
  const reasoningShare =
    metrics.outputTokens > 0 && metrics.reasoningReported
      ? metrics.reasoningTokens / metrics.outputTokens
      : null;
  ...
  return {
    ...
    reasoningShare,
    reasoningReported: metrics.reasoningReported,
    avgReasoningTokens,
    cacheSavingUsd,
