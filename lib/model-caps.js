...
export function createCapabilityCache({ resolveModelInfo, negativeTtlMs = 30_000, maxEntries = 128 } = {}) {
  /** @type {Map<string, {resolvable: boolean, supported: string[]|null, contextWindow: number|null, at: number}>} */
  const cache = new Map();
  ...
  return {
    /**
     * 问出某个路由的能力：可解析性、支持的推理档位、**上下文窗口**。
     *
     * 上下文窗口来自 `LlmResolvedModelInfo.context.contextWindow`
     * （见 llm 服务的类型声明）。它用来防止"切到一个装不下当前会话的模型"——
     * 那种切换的后果不是花钱，而是**请求必然失败**（CONTEXT_WINDOW_EXCEEDED）。
     *
     * @returns {Promise<{resolvable: boolean, supported: string[]|null, contextWindow: number|null, cached: boolean}>}
     */
    async capabilities(provider, model) {
      ...
      if (typeof resolveModelInfo !== 'function') {
        return { resolvable: false, supported: null, contextWindow: null, cached: false };
      }
      try {
        ...
        const supported =
          Array.isArray(efforts) && efforts.length > 0
            ...
            : null;
        // 窗口只认正数：0/NaN/缺失一律当作"未知"，由策略层决定如何保守处理。
        const raw = info?.context?.contextWindow;
        const contextWindow = typeof raw === 'number' && Number.isFinite(raw) && raw > 0 ? raw : null;
        remember(key, { resolvable: true, supported, contextWindow, at: Date.now() });
        return { resolvable: true, supported, contextWindow, cached: false };
      } catch {
        remember(key, { resolvable: false, supported: null, contextWindow: null, at: Date.now() });
        return { resolvable: false, supported: null, contextWindow: null, cached: false };
      }
    },
