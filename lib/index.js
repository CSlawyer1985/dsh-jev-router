...
export function apply(ctx, config) {
  ...
  function sessionFor(id) {
    return sessions.forSession(id, () => ({
      ...
      jevFailure: null,
      /** Tier B 本轮是否已经写过 model/selection（避免重复写）。 */
      selectionSyncedThisTurn: false,
      /** 最近一次把档位同步进会话配置的时间。 */
      effortSyncedAt: null,
      /** 同步失败原因（会话不可追加时）。 */
  ...
  ctx.on('agent/request', async (payload, next) => {
    ...
    if (atTurnStart && C('modelRouting') === true) {
      const before = `${replacement.provider}/${replacement.model}`;
      await applyModelRouting({ agent, entry, replacement });
      // Tier B 的切换本身已经写过 model/selection（并带上了档位），
      // 记下来避免下面再写一次重复事件。
      entry.selectionSyncedThisTurn = `${replacement.provider}/${replacement.model}` !== before;
    }

    // Tier A 的档位同步：让 DSH **原生的档位显示**跟随。
    //
    // 为什么必须有：Tier A 只改 LlmCallConfig.reasoningEffort —— 对 DSH 来说
    // 会话配置没变，于是界面上「模型后面那个思考程度」永远停在会话设置上。
    // 用户看不到任何变化，就会认为插件没工作（这正是实际发生的反馈）。
    //
    ...
    if (
      atTurnStart &&
      !entry.selectionSyncedThisTurn &&
      C('syncSessionEffort') !== false &&
      entry.lastEffortVerdict?.changed === true &&
      replacement.reasoningEffort != null
    ) {
      try {
    ...
    entry.selectionSyncedThisTurn = false;
