...
export function apply(ctx, config) {
  ...
  ctx.on('agent/inbox/inserted', (payload) => {
    ...
    //   · agent-instructions   workspace 指令（<system-reminder>）
    //   · runtime-context      运行时快照（"Current runtime context…"）
    //   · goal / tool-jobs / user-approval …
    // 它们的文本无关用户意图，发给 Jev 既浪费调用，又会与用户消息
