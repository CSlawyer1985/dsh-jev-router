...
export function apply(ctx, config) {
  ...
  async function applyModelRouting({ agent, entry, replacement }) {
    ...
    const verdict = shouldSwitchModel({
      ...
      // 上下文闸用**未命中 inputTokens**，不是 input+cacheRead。
      // 当前 Codex provider 的真实行为是：cached prefix 可以超过 272K，
      // 但新的 uncached input 超过窗口会触发 overflow。`prefixTokens` 仍只给成本闸用。
      usedTokens: estimateUncachedTokens(metrics),
    });
...
/**
 * 上下文闸使用未命中的 inputTokens。
 *
 * 当前 Codex provider 的真实 usage 事实：cached prefix 可以达到 552K+
 * 而 uncached input 只有 57K 时请求成功；把两者相加会错误拦掉合法的热缓存请求。
 */
function estimateUncachedTokens(metrics) {
  return metrics.lastStep ? (metrics.lastStep.inputTokens ?? 0) : 0;
}

export const __test__ = { estimatePrefixTokens, estimateUncachedTokens, averageReasoningTokens, EFFORTS };
