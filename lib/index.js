...
export function apply(ctx, config) {
  ...
  async function applyModelRouting({ agent, entry, replacement }) {
    ...
    const verdict = shouldSwitchModel({
      ...
      // 上下文闸必须用 **input + cacheRead**（即完整 prompt 规模）。
      //
      // 依据是 pi-ai 的 isContextOverflow Case 2（逐字核对过）：
      //   if (contextWindow && message.stopReason === "stop") {
      //     const inputTokens = message.usage.input + message.usage.cacheRead;
      //     if (inputTokens > contextWindow) return true;
      //   }
      // 也就是说即使服务端没报错，只要「输入总量 > 窗口」就会被判为溢出。
      //
      // 曾经的错误修正：看到一次 in=57K + cache=552K 的请求"成功"就改成只看 input，
      // 但那段时间其实是 DSH 的 compaction 在反复压缩重试（日志里 7 次
