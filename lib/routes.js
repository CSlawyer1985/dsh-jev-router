  ...
  disposers.push(
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          // 会话作用域的前端会带上 ?session=<id>，据此精确定位；
          // 不带（例如 root 作用域的设置页）才回落到"最近活跃会话"。
          let sessionId = null;
          try {
            sessionId = new URL(request.url ?? '/', 'http://localhost').searchParams.get('session');
          } catch {
            /* query 解析失败就当作未提供 */
          }
          const target = sessionId || firstAgent(runtime);
          // snapshotForClient 是 async 的——必须 await，
          // 否则 JSON.stringify(Promise) 会静默产出 "{}"。
          sendJson(response, 200, await runtime.snapshotForClient(target));
        } catch (error) {
