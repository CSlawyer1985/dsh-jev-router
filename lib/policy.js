 ...
 */
export function decideEffort({ state, config, decided, confidence, currentHarnessEffort, trusted = false }) {
  ...
  // 降档需要连续确认，防止一次误判把强度砍掉。
  // 例外：关键词回退是**用户自己写下的字面指令**（「快速回答」/「ultrathink」），
  // 不确认才符合直觉——否则整个回退路径的价值会被削掉一半。
  const isDowngrade = EFFORTS.indexOf(decided) < EFFORTS.indexOf(base);
  if (isDowngrade && !trusted) {
    state.lowStreak += 1;
