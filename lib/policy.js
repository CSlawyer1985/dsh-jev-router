 ...
 * @param {number|null} [input.usedTokens] 当前请求的**未命中 inputTokens**（token）
...
export function shouldSwitchModel({
  ...
  //
  // 实测事故：一个已累积 556K、且当时 cacheRead=0 的会话切到窗口为 272K 的模型，
  // 请求直接被 pi-ai 拒绝。另一次热缓存请求虽然总量约 610K（input 57K + cache 552K），
  // 却成功——说明当前 Codex provider 的 overflow 边界按未命中 input 走，
  // cached prefix 由 provider 的缓存路径单独处理。
  // 所以这里刻意比较 uncached input，而不是把 cacheRead 也加进去。
  //
